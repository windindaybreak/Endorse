...
public class BillInfo extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ListView listView = findViewById( R.id.bill_info );
        List<IntroductionItem> list = new ArrayList<>();
        IntroductionItem item;
        reply = con.CSQI( "604", cookie, "000" ); // 400# company not found
        int l = 1, r = 1;
        ...
        item=new IntroductionItem(  "纳税人识别号:",reply.substring( l, r)  );
        list.add( item );
        l = r + 1;
        for (int i = l; i < reply.length(); i++)
            if (reply.charAt( i ) == '#') {
        ...
        item=new IntroductionItem(  "地址:",reply.substring( l, r)  );
        list.add( item );
        l = r + 1;
        for (int i = l; i < reply.length(); i++)
            if (reply.charAt( i ) == '#') {
        ...
        item=new IntroductionItem(  "电话:",reply.substring( l, r)  );
        list.add( item );
        l = r + 1;
        for (int i = l; i < reply.length(); i++)
            if (reply.charAt( i ) == '#') {
        ...
        item=new IntroductionItem(  "开户银行:",reply.substring( l, r)  );
        list.add( item );
        l = r + 1;
        for (int i = l; i < reply.length(); i++)
            if (reply.charAt( i ) == '#') {
        ...
        item=new IntroductionItem(  "账号:",reply.substring( l, r)  );
        list.add( item );
        l = r + 1;
        ItemAdapter adapter = new ItemAdapter( BillInfo.this, R.layout.info_item, list );
    ...
    public void onWindowFocusChanged(boolean hasFocus) {
        ...
        if (reply.substring( 0, 3 ).equals( "400" )) {
            AlertDialog.Builder builder = new AlertDialog.Builder( BillInfo.this );
            ...
            builder.setPositiveButton( "重新登陆", new DialogInterface.OnClickListener() {
                ...
                public void onClick(DialogInterface dialog, int which) {
                    Intent intent = new Intent( BillInfo.this, LoginActivity.class );
